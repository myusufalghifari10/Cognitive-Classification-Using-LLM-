#!/usr/bin/env bash
# 02_finetune.sh — FINE-TUNE (QLoRA) + MERGE + CONVERT GGUF Q8. SATU PASANG.
#
...
echo ""
echo "═══════════════════════════════════════════════════════"
echo " [1/4] TRAIN (QLoRA 4-bit, batch=1 grad-accum=16, seq=8192)"
echo "═══════════════════════════════════════════════════════"
rm -rf "$SCRATCH"; mkdir -p "$SCRATCH"
# config 48GB: 4-bit base (~18GB) + MoE 3B active → aktivasi kecil. seq 8192 (max data ~8452 tok).
python train.py \
  --model "$MODEL_PATH" \
